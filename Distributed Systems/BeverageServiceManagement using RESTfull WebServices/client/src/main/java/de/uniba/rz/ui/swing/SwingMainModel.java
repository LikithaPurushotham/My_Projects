...
public class SwingMainModel extends Observable {
    ...
    public List<Ticket> searchTicket(String page, String limit, String name, Type type) throws TicketException {
		try {
			if (type == null) {
				return backend.getTicketsByName(page, limit, name);
			} else {
				return backend.getTicketsByNameAndType(page, limit, name, type);
			}
