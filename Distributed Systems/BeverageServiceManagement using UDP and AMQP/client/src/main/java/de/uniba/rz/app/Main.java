...
import de.uniba.rz.app.RabbitMQ.MQTicketManagementBackend;
...
import de.uniba.rz.app.UDP_Client.UdpTicketClient;
...
public class Main {
	...
	private static TicketManagementBackend evaluateArgs(String[] args) {
		if (args == null || args.length == 0) {
			...
			case "udp":
				if(args.length!=2){
					System.out.println("Need host address as argument. Returning local implementation");
					return new LocalTicketManagementBackend();
				}
				return new UdpTicketClient(args[1]);
			case "amqp":
			    if(args.length!=3){
			        System.out.println("Either hostname or routingKey missing. Returning local implementation");
                    return new LocalTicketManagementBackend();
                }
				return new MQTicketManagementBackend(args[1],args[2]);
