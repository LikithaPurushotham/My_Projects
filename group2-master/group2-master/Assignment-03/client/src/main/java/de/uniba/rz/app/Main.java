...
public class Main {
	 ...
	 *
	 * @param args
	 * args[0] "grcp" if the required backend is GRPCTicketManagementBackend
	 * args[1] host address for grcp server
	 * args[2] port number
	 */
	public static void main(String[] args) {
		...
		//Check if the backend management is of GRPCTicketManagementBackend class
		//so that we can send swingMainController to manage auto refresh of tickets table
		if(backendToUse instanceof GRPCTicketManagementBackend){
			((GRPCTicketManagementBackend) backendToUse).initiateGRPCAutoUpdate(control);
		}
	...
	private static TicketManagementBackend evaluateArgs(String[] args) {
		if (args == null || args.length == 0) {
			...
			case "grpc":
				if(args.length!=3){
					System.out.println("Either server or port is not passed. Using local backend implementation");
					return new LocalTicketManagementBackend();
				}
				return new GRPCTicketManagementBackend(args[1],Integer.parseInt(args[2]));
